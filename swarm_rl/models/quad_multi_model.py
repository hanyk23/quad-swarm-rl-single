...
from swarm_rl.vision.yolo_obstacles import YOLO_OBS_DIM


def get_obstacle_obs_dim(cfg):
    if cfg.quads_obstacle_obs_type == "yolo":
        return YOLO_OBS_DIM
    if cfg.quads_obstacle_obs_type == "lidar":
        return int(cfg.quads_lidar_num_rays)
    if cfg.quads_obstacle_obs_type == "depth":
        return int(cfg.quads_depth_grid_width) * int(cfg.quads_depth_grid_height)
    return QUADS_OBSTACLE_OBS_TYPE[cfg.quads_obstacle_obs_type]
...
class QuadNeighborhoodEncoderAttention(QuadNeighborhoodEncoder):
    ...
    def forward(self, self_obs, obs, all_neighbor_obs_size, batch_size):
        ...
        attention_weights = self.attention_mlp(attention_mlp_input).view(batch_size, -1)
...
class QuadMultiHeadAttentionEncoder(Encoder):
    def __init__(self, cfg, obs_space):
        ...
        self.obstacle_obs_dim = get_obstacle_obs_dim(cfg)
        ...
        self.feed_forward = nn.Sequential(
            fc_layer(3 * cfg.rnn_size, self.encoder_output_size),
            nn.Tanh(),
        )
    ...
    def forward(self, obs_dict):
        ...
        attn_embed, _ = self.attention_layer(attn_embed, attn_embed, attn_embed)
...
class QuadMultiEncoder(Encoder):
    def __init__(self, cfg, obs_space):
        ...
        if num_use_neighbor_obs > 0:
            ...
            if neighbor_encoder_type == "mean_embed":
                self.neighbor_encoder = QuadNeighborhoodEncoderDeepsets(
                    ...
                    neighbor_hidden_size=neighbor_hidden_size,
                    self_obs_dim=self.self_obs_dim,
                    num_use_neighbor_obs=num_use_neighbor_obs,
                )
        ...
        self_encoder_out_size = calc_num_elements(
            self.self_encoder,
            (self.self_obs_dim,),
        )
        ...
        if self.use_obstacles:
            obstacle_obs_dim = get_obstacle_obs_dim(cfg)
        ...
        total_encoder_out_size = (
            self_encoder_out_size
            + neighbor_encoder_out_size
            + obstacle_encoder_out_size
        )
    ...
    def forward(self, obs_dict):
        ...
        if self.neighbor_encoder:
            neighborhood_embedding = self.neighbor_encoder(
                obs_self,
                obs,
                self.all_neighbor_obs_size,
                batch_size,
            )
